...
class HeartRateVariabilityAdjustmentTest {

    private val start = Instant.parse("2024-01-01T00:00:00Z")
    private val step = Predictor.TIME_SERIES_STEP_DURATION

    @Test
    fun `hrv is dampened when heart rate exceeds threshold`() {
        val raw = Predictor.MultiTimeSeriesEntries(
            timeStart = start,
            duration = step,
            heartRate = listOf(
            ...
            heartRateVariability = listOf(
                HeartRateVariabilityEntry(start, 100.0)
            ),
            distance = emptyList(),
            elevationGained = emptyList(),
            skinTemperature = emptyList(),
        ...
        val mts = MultiTimeSeriesDiscrete.fromEntries(
            raw,
            Predictor.FixedParameters(anaerobicThresholdBPM = 140.0)
        )

        val expected = 100.0 * 0.8
        val hrv = mts[
            MultiTimeSeriesDiscrete.FeatureID(
        ...
        assertEquals(expected, hrv, 1e-6)
    }

    @Test
    fun `hrv damping is capped at 0_5`() {
        val raw = Predictor.MultiTimeSeriesEntries(
            timeStart = start,
            duration = step,
            heartRate = listOf(
                HeartRateEntry(start, 260L)
            ),
            heartRateVariability = listOf(
                HeartRateVariabilityEntry(start, 80.0)
            ),
            distance = emptyList(),
            elevationGained = emptyList(),
            skinTemperature = emptyList(),
        ...
        assertEquals(80.0 * 0.5, hrv, 1e-6)
    }
}
