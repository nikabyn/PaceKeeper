...
suspend fun exportAllAsZip(db: PacingDatabase, outputStream: OutputStream) =
    withContext(Dispatchers.IO) {
        ZipOutputStream(outputStream).use { zipOut ->
            ...
            exportEntityToCsv("manual_symptom.csv", zipOut) {
                db.manualSymptomDao().getAll().map {
                    CsvRow(
                        it.feeling.time.toString(), mapOf(
                            "percentageNow" to it.feeling.feeling.ordinal.toString(),
                            "timeFuture" to it.symptoms.joinToString (separator = ","){ symptom -> symptom.name },
                        )
                    )
                }
            }
