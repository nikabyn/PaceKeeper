...
class DataExportService(private val db: PacingDatabase) {

    suspend fun exportAllAsZip(outputStream: OutputStream) = withContext(Dispatchers.IO) {
        ZipOutputStream(outputStream).use { zipOut ->

            exportEntityToCsv("heart_rate.csv", zipOut) {
                db.heartRateDao().getAll().map {
                    CsvRow(it.time.toEpochMilliseconds(), mapOf("bpm" to it.bpm.toString()))
                }
            }

            exportEntityToCsv("distance.csv", zipOut) {
                db.distanceDao().getAll().map {
                    CsvRow(it.start.toEpochMilliseconds(), mapOf("distanceMeters" to it.length.inMeters().toString()))
                }
            }

            exportEntityToCsv("elevation.csv", zipOut) {
                db.elevationGainedDao().getAll().map {
                    CsvRow(it.start.toEpochMilliseconds(), mapOf("elevationMeters" to it.length.inMeters().toString()))
                }
            }

            exportEntityToCsv("energy_level.csv", zipOut) {
                db.energyLevelDao().getAll().map {
                    CsvRow(it.time.toEpochMilliseconds(), mapOf("energyLevel" to it.percentage.toString()))
                }
            }

            exportEntityToCsv("hrv.csv", zipOut) {
                db.heartRateVariabilityDao().getAll().map {
                    CsvRow(it.time.toEpochMilliseconds(), mapOf("hrv" to it.variability.toString()))
                }
            }

            exportEntityToCsv("menstruation.csv", zipOut) {
                db.menstruationPeriodDao().getAll().map {
                    CsvRow(it.start.toEpochMilliseconds(), mapOf("end" to it.end.toEpochMilliseconds().toString()))
                }
            }

            exportEntityToCsv("oxygen_saturation.csv", zipOut) {
                db.oxygenSaturationDao().getAll().map {
                    CsvRow(it.time.toEpochMilliseconds(), mapOf("saturation" to it.percentage.toString()))
                }
            }

            exportEntityToCsv("skin_temperature.csv", zipOut) {
                db.skinTemperatureDao().getAll().map {
                    CsvRow(it.time.toEpochMilliseconds(), mapOf("tempCelsius" to it.temperature.inCelsius().toString()))
                }
            }

            exportEntityToCsv("sleep_sessions.csv", zipOut) {
                db.sleepSessionsDao().getAll().map {
                    CsvRow(it.start.toEpochMilliseconds(), mapOf(
            ...
            exportEntityToCsv("speed.csv", zipOut) {
                db.speedDao().getAll().map {
                    CsvRow(it.time.toEpochMilliseconds(), mapOf("speed" to it.velocity.inMetersPerSecond().toString()))
                }
            }
