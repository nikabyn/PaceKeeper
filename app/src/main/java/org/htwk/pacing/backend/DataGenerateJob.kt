...
object DataGenerateJob {
    ...
    private suspend fun checkAndImportDemoData(db: PacingDatabase) {
        val thirtyDaysAgo = Clock.System.now() - 30.days
        val hasRecentData = db.readEventDao().getAll().any { it.time > thirtyDaysAgo }

        if (!hasRecentData) {
            val demoPath = "/storage/emulated/0/Download/pacing_export (4).zip"
            storeDemoRecords(db, demoPath)
        }
    }

    private suspend fun performDataGeneration(db: PacingDatabase) {
