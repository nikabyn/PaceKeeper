...
import org.htwk.pacing.predictor.preprocessing.MultiTimeSeriesDiscrete.Companion.featureCount
import org.htwk.pacing.predictor.preprocessing.MultiTimeSeriesDiscrete.Companion.stepDuration
...
class MultiTimeSeriesDiscrete(val timeStart: Instant, initialCapacityInSteps: Int = 512) {
     ...
     * @return The calculated [kotlinx.datetime.Instant] corresponding to the sample index.
     ...
     * @return A new [org.jetbrains.kotlinx.multik.ndarray.data.D1Array] containing the values of all features at the specified `step`.
    ...
    companion object {
        ...
        fun fromEntries(raw: Predictor.MultiTimeSeriesEntries): MultiTimeSeriesDiscrete {
            ...
            TimeSeriesMetric.entries.forEach { metric ->
                ...
                val discreteProportional = TimeSeriesDiscretizer.discretizeTimeSeries(
                    ensureData(
                        id = metric.ordinal,
                        GenericTimedDataPointTimeSeries(
                            ...
                            data = when (metric) {
                                TimeSeriesMetric.HEART_RATE -> raw.heartRate.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )

                                TimeSeriesMetric.DISTANCE -> raw.distance.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )

                                TimeSeriesMetric.ELEVATION_GAINED -> raw.elevationGained.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )

                                TimeSeriesMetric.SKIN_TEMPERATURE -> raw.skinTemperature.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )
                                ...
                                TimeSeriesMetric.OXYGEN_SATURATION -> raw.oxygenSaturation.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )

                                TimeSeriesMetric.STEPS -> raw.steps.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )

                                TimeSeriesMetric.SPEED -> raw.speed.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )

                                TimeSeriesMetric.SLEEP_SESSION -> raw.sleepSession.map(
                                    GenericTimedDataPointTimeSeries::GenericTimedDataPoint
                                )
                            }
                        ).also { it ->
